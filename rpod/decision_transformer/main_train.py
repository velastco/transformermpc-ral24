...
# Initial parameters
model_name = 'checkpoint_rtn_ctgrtg'
...
datasets, dataloaders = ART_manager.get_train_val_test_data(state_representation=state_representation, dataset_to_use=dataset_to_use, mdp_constr=mdp_constr, model_name=model_name, timestep_norm=timestep_norm)
...
def evaluate():
    ...
    for step in range(eval_iters):
        ...
        states_i, actions_i, rtgs_i, ctgs_i, goal_i, timesteps_i, attention_mask_i, _, _, _, _, _ = next(data_iter)
        with torch.no_grad():
...
for epoch in range(num_train_epochs):
    for step, batch in enumerate(train_dataloader, start=0):
        with accelerator.accumulate(model):
            states_i, actions_i, rtgs_i, ctgs_i, goal_i, timesteps_i, attention_mask_i, _, _, _, _, _ = batch
            state_preds, action_preds = model(
                states=states_i,
                actions=actions_i,
                goal=goal_i,
            ...
            if (step % (eval_steps*10)) == 0:
                print('Saving model..')
                accelerator.save_state(root_folder+'/decision_transformer/saved_files/checkpoints/'+model_name)
                np.savez_compressed(root_folder + '/decision_transformer/saved_files/checkpoints/' +model_name+ '/log',
                            log = log
                            )