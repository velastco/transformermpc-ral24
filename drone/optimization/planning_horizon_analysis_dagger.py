...
from dynamics.quadrotor import sample_init_target, ocp_no_obstacle_avoidance, compute_constraint_to_go, QuadModel, obs_positions, obs_radii
import torch
import time
from dynamics.QuadEnv import QuadEnv
import optimization.quad_scenario as quad
from decision_transformer.art_closed_loop import AutonomousQuadrotorTransformerMPC, ConvexMPC, MyopicConvexMPC
...
def for_computation(input_iterable):
    ...
    dataset_scenario = input_dict['dataset_scenario']
    ...
    if sample_init_final:
        ...
        test_sample[0][0,:,:] = (torch.tensor(np.repeat(state_init[None,:], quad.n_time_rpod, axis=0)) - data_stats['states_mean'])/(data_stats['states_std'] + 1e-6)
        test_sample[1][0,:,:] = torch.zeros((quad.n_time_rpod,quad.n_u))
        test_sample[2][0,:,0] = torch.zeros((quad.n_time_rpod,))
        test_sample[3][:,0] = torch.zeros((quad.n_time_rpod,))
        test_sample[4][0,:,:] = (torch.tensor(np.repeat(state_final[None,:], quad.n_time_rpod, axis=0)) - data_stats['goal_mean'])/(data_stats['goal_std'] + 1e-6)
    ...
    if not np.char.equal(feas_cvx,'infeasible'):
        ...
        ctgs_cvx = compute_constraint_to_go(states_cvx, obs_positions, obs_radii)
        ...
        if not out['cvx_problem']:
            ...
            env_cvxMPC = QuadEnv(dataset_scenario)
            ...
            try:
                time_cvxMPC = np.empty((quad.n_time_rpod,))
                for i in np.arange(quad.n_time_rpod):         
                    # CVX-ws
                ...
                out['J_cvxMPC'] = np.sum(la.norm(env_cvxMPC.dv, axis=0)**2)/2
                out['J_com_cvxMPC'] = np.sum(la.norm(env_cvxMPC.dv, axis=0)**2)/2
                out['time_cvxMPC'] = time_cvxMPC
            except:
                out['feasible_cvxMPC'] = False
            
            env_myocvxMPC = QuadEnv(dataset_scenario)
            env_myocvxMPC.reset('det', reset_condition=(dt, state_init, state_final))
            myocvxMPC = MyopicConvexMPC(mpc_steps, scp_mode='soft')
            # For cycle one by one to isolate failures: Type error means the optimizer failed and results are None -> TypeError
            try:
                time_myocvxMPC = np.empty((quad.n_time_rpod,))
                for i in np.arange(quad.n_time_rpod):         
                    # CVX-ws
                ...
                out['J_myocvxMPC'] = np.sum(la.norm(env_myocvxMPC.dv, axis=0)**2)/2
                out['J_com_myocvxMPC'] = np.sum(la.norm(env_myocvxMPC.dv, axis=0)**2)/2
                out['time_myocvxMPC'] = time_myocvxMPC
            except:
                out['feasible_myocvxMPC'] = False

            env_artMPC = QuadEnv(dataset_scenario)
            env_artMPC.reset('det', reset_condition=(dt, state_init, state_final))
            artMPC = AutonomousQuadrotorTransformerMPC(models[0], test_loader, mpc_steps, transformer_mode='dyn', ctg_clipped=True, scp_mode='soft')
            try:
                time_artMPC = np.empty((quad.n_time_rpod,))
                for i in np.arange(quad.n_time_rpod):
                    # ART-ws
                ...
                out['J_artMPC'] = np.sum(la.norm(env_artMPC.dv, axis=0)**2)/2
                out['J_com_artMPC'] = np.sum(la.norm(env_artMPC.dv, axis=0)**2)/2
            ...
            for n_model in np.arange(len(models)-1):
                env_artMPC = QuadEnv(dataset_scenario)
                env_artMPC.reset('det', reset_condition=(dt, state_init, state_final))        
                artMPC = AutonomousQuadrotorTransformerMPC(models[n_model+1], test_loader, mpc_steps, transformer_mode='dyn', ctg_clipped=True, scp_mode='soft')
                try:
                    time_artMPC = np.empty((quad.n_time_rpod,))
                    for i in np.arange(quad.n_time_rpod):
                        # ART-ws
                        ...
                        #env_artMPC.load_prediction(art_traj, artMPC_traj)
                        _ = env_artMPC.step(artMPC_traj['dv'][:,0])
                    out['J_artMPC_dag'+str(n_model)] = np.sum(la.norm(env_artMPC.dv, axis=0)**2)/2
                    out['J_com_artMPC_dag'+str(n_model)] = np.sum(la.norm(env_artMPC.dv, axis=0)**2)/2
...
if __name__ == '__main__':
    ...
    model_name_import = 'checkpoint_quad_random_forest_ctgrtg'
    ...
    for n_steps in mpc_steps_analysis:
        ...
        time_cvxMPC = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
        time_myocvxMPC = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
        time_artMPC = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
        time_artMPC_dag0 = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
        time_artMPC_dag1 = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
        time_artMPC_dag2 = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
        time_artMPC_dag3 = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
        time_artMPC_dag4 = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
        time_artMPC_dag5 = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
        time_artMPC_dag6 = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
        time_artMPC_dag7 = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
        time_artMPC_dag8 = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
        time_artMPC_dag9 = np.empty(shape=(N_data_test, quad.n_time_rpod), dtype=float)
