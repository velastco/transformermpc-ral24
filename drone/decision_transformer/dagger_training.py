...
from decision_transformer.art_closed_loop import AutonomousQuadrotorTransformerMPC, ConvexMPC
import optimization.quad_scenario as quad
from dynamics.quadrotor import QuadModel, ocp_no_obstacle_avoidance, compute_constraint_to_go
import time
...
def for_computation(input_iterable):
    ...
    try:
        states_cvx, actions_cvx, _, feas_cvx = ocp_no_obstacle_avoidance(quad_model, state_init, state_final, initial_guess='line') #### Remember states_cvx:(101,6) actions_cvx:(100,3)
    ...
    if not np.char.equal(feas_cvx,'infeasible'):
        ...
        rtg_0 = -out['J_cvx']
        ctgs_cvx = compute_constraint_to_go(states_cvx, quad.obs_positions, quad.obs_radii)
        ...
        quad_env_art = QuadEnv(quad.dataset_scenario)
        ...
        artMPC = AutonomousQuadrotorTransformerMPC(model,train_loader,mpc_steps,transformer_mode='dyn',ctg_clipped=True,scp_mode='soft')
        ...
        try:
            ...
            out['J_artMPC'] = np.sum(la.norm(quad_env_art.dv, axis=0)**2)/2
            ...
            out['target_state'] = torch.tensor(np.array([correction_pred_history[t]['state_CVXMPC'][:, 1] for t in range(quad.n_time_rpod-1)]))
            out['target_action'] = torch.tensor(np.array([correction_pred_history[t]['dv_CVXMPC'][:, 0] for t in range(quad.n_time_rpod)]))
...
if __name__ == '__main__':
    ...
    initial_transformer_name = 'checkpoint_quad_random_forest_ctgrtg'
    ...
    dataset_scenario = import_config_ol['dataset_scenario']
    train_only_on_cl = False
    oracle_fixed_flag = True
    # Get the datasets and loaders from the torch data
    datasets_ol, dataloaders_ol = ART_manager.get_train_val_test_data(mdp_constr, dataset_scenario, timestep_norm)
    ...
    for n_dagger in range(n_dagger_i, N_dagger_max):
        ...
        context_state = np.empty(shape=(N_data_test, quad.n_time_rpod, 6), dtype=float)
        context_state_norm = np.empty(shape=(N_data_test, quad.n_time_rpod, 6), dtype=float)
        context_action = np.empty(shape=(N_data_test, quad.n_time_rpod, 3), dtype=float)
        context_action_norm = np.empty(shape=(N_data_test, quad.n_time_rpod, 3), dtype=float)
        target_state = np.empty(shape=(N_data_test, quad.n_time_rpod-1, 6), dtype=float)
        target_action = np.empty(shape=(N_data_test, quad.n_time_rpod, 3), dtype=float)
        context_rtg = np.empty(shape=(N_data_test, quad.n_time_rpod, 1), dtype=float)
        context_ctg = np.empty(shape=(N_data_test, quad.n_time_rpod, 1), dtype=float)
        context_goal = np.empty(shape=(N_data_test, quad.n_time_rpod, 6), dtype=float)
        context_goal_norm = np.empty(shape=(N_data_test, quad.n_time_rpod, 6), dtype=float)
